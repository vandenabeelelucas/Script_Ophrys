...
plot.weights <- function(weights_dataframe,positions=NULL,line_cols=NULL,fill_cols=NULL,density=NULL,lwd=1,xlim=NULL,ylim=c(0,1),stacked=FALSE,
                         ylab="Weighting", xlab = "", main="",xaxt=NULL,yaxt=NULL,bty="n", add=FALSE){
  #get x axis
  x = positions
  ...
  #set x limits
  # Modification: use real genomic positions for xmax
  if(is.null(xlim)) xlim = c(min(x), max(positions))  
  
  #if not adding to an old plot, make a new plot
  if (add==FALSE) {
    ## Modification: disable default x-axis in order to draw a custom x-axis
    plot(0, pch = "", xlim = xlim, ylim=ylim, ylab=ylab, xlab=xlab, main=main, xaxt="n", yaxt=yaxt, bty=bty)
    # Flatten all genomic coordinates
    all_positions <- unlist(positions)
    # Generate aesthetically spaced tick marks
    ticks <- pretty(all_positions)
    # Remove ticks outside genomic range
    ticks <- ticks[ticks >= min(positions) & ticks <= max(positions)]
    # Draw labels without the horizontal axis line
    axis(1, at = ticks, labels = ticks, lwd = 0)
    # Draw small vertical tick marks manually
    tick_height <- diff(par("usr")[3:4]) * 0.02
    segments(x0 = ticks, y0 = par("usr")[3], 
             x1 = ticks, y1 = par("usr")[3] + tick_height, 
             col = "black", lwd = 1)
  }
  ...
  # Modification: draw a baseline at y = 0 across the full genomic region
  lines(c(min(positions), max(positions)), c(0,0), col="black", lwd=1)
}
